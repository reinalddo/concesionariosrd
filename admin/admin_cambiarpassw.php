...
function Insertar()
{
	...
	{
		acepto = false;
		alert("CONTRASEÑA ANTERIOR OBLIGATORIA");
	}
...
if(isset($_POST['anterior']))
{
	...
	if($obj_usuarios->Verificar_Clave_Anterior($usuario, $anterior))
	{
		$obj_usuarios->Cambiar_Passw($usuario, $nueva);
		$band = 1;
	}
	else 
	{
...
?>
  <tr>
    ...
    <td>
      <select class="input input-lg" name="otro_usuario" id="otro_usuario" onchange="document.cambio.action = 'cambiarpassw_otro.php';document.cambio.submit();">
		...
		{
		?>
        <option value="<?php echo $row_usuarios_marca["LOGIN"]; ?>"><?php echo $row_usuarios_marca["LOGIN"]." - ".$row_usuarios_marca["NOMBRE"]." ".$row_usuarios_marca["APELLIDO"]; ?></option>
        <?php
		}
		?>
      </select>
...
?>
  ...
  <tr>
    <td style="font-weight:bold">Contraseña Anterior: &nbsp;</td>
    <td>
      <input class="input input-lg" name="anterior" type="password" id="anterior" />
     </td>
  </tr>
  <tr>
